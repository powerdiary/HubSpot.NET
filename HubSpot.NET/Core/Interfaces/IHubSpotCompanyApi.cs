...
namespace HubSpot.NET.Core.Interfaces
{
    public interface IHubSpotCompanyApi
    {
        ...
        CompanySearchResultModel<T> GetByDomain<T>(string domain, CompanySearchByDomain options = null)
            where T : CompanyHubSpotModel, new();

        Task<CompanySearchResultModel<T>> GetByDomainAsync<T>(string domain, CompanySearchByDomain options = null)
            where T : CompanyHubSpotModel, new();

        CompanyListHubSpotModel<T> List<T>(ListRequestOptions opts = null) where T : CompanyHubSpotModel, new();

        Task<CompanyListHubSpotModel<T>> ListAsync<T>(ListRequestOptions opts = null)
            where T : CompanyHubSpotModel, new();
        ...
        Task<CompanySearchHubSpotModel<T>> SearchAsync<T>(SearchRequestOptions opts = null)
            where T : CompanyHubSpotModel, new();
