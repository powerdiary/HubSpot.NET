...
public class HubSpotContactListApi : IHubSpotContactListApi
{
    ...
    public Task<ContactListUpdateResponseModel> RemoveContactsFromListAsync(long listId, IEnumerable<long> contactIds)
    {
        var model = new ContactListUpdateModel();
        var path = $"{model.RouteBasePath}/{listId}/remove";
        model.ContactIds.AddRange(contactIds);

        return _client.ExecuteAsync<ContactListUpdateResponseModel>(path, model, Method.POST, convertToPropertiesSchema: false);
    }
